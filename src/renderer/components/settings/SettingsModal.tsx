...
import { Bot, Info, Languages, Palette, Power, Search, TerminalSquare, type LucideIcon } from 'lucide-react'
...
type CategoryId = 'appearance' | 'terminal' | 'language' | 'agents' | 'startup' | 'about'
...
const CATEGORIES: Category[] = [
  ...
  { id: 'agents', labelKey: 'settings.cat.agents', icon: Bot, keywords: ['preset', 'agent', 'agente', 'claude', 'codex'] },
  { id: 'startup', labelKey: 'settings.cat.startup', icon: Power, keywords: ['startup', 'inicio', 'tray', 'bandeja', 'launch', 'close', 'cerrar'] },
...
export function SettingsModal() {
  ...
  const current = visible.some((c) => c.id === active) ? active : (visible[0]?.id ?? 'appearance')
